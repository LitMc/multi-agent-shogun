...
| **エージェント一覧** | 9ウィンドウ同時監視。タップで全画面展開 + コマンド送信 |
...
- 家老がタスク割当・完了時に自動更新
- 9ウィンドウを切替（Ctrl-b n/p）すれば、誰が何をしているか即座にわかる
...
│      ├──▶ tmuxセッションを作成                                       │
│      │         • "shogun"セッション（1ペイン）                        │
│      │         • "multiagent"セッション（9ウィンドウ、各エージェント独立） │
│      │                                                              │
...
alias csm='tmux attach-session -t multiagent'  # 9ウィンドウ表示（Ctrl-b n/pで切替）
```
