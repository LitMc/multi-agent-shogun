...
| Might and Magic VI: The Mandate of Heaven (1998) | Windows 95/98 | GrayFace Patch適用推奨。ソフトウェアレンダリング専用（GPU非依存） |
...
| パーツ | 型番 | 状態 |
|--------|------|------|
| CPU | Intel Pentium II 400MHz (Slot 1, Deschutes, FSB 100MHz) | 落札中(eBay) |
| MB | ASUS P2B / P3B-F (440BX) | 未購入 |
| GPU | Matrox Millennium G200 AGP | 購入済み |
| Sound | Creative SB Live! Value CT4830 (EMU10K1) | 購入済み |
...
**★MTG Shandalarの速度問題について（cmd_306-308調査結果）★**
- MTGはCPUクロック依存でゲーム速度が変わるバグあり。快適ゾーン: 66-120MHz
- PII 400MHzでは確実に速すぎる。PII 233MHz（440BX最低）でも快適ゾーン外（IPC差でP5 315-350MHz相当）
- cpukillerは非実用的（VOGONSで「very laggy」報告）。コミュニティパッチでの速度修正もなし
- **解法: MTGは86Box（Pentium 100-133MHz設定）で対応**。実機はMM6専用機として運用
- PII 233MHz(Klamath)での実機シャンダラーは保留。86Boxでテストしてから購入判断

**★MM6のGPU要件（cmd_309調査結果）★**
- MM6はソフトウェアレンダリング専用（開発元が意図的に選択）。3Dカード不要
- G200の2D性能でMM6は快適に動作する。追加GPU購入不要
- 3Dゲームも遊びたくなった場合: G400差し替え（¥1,000-5,000）またはTNT2差し替え（¥1,500-6,000）
...
| 決定項目 | 決定内容 | 根拠 |
|---|---|---|
| **実機構成** | PII 400MHz + G200 + 440BX (P2B/P3B-F) | MM6 + Win98ゲーム用。構成確定(2026-03-27) |
| **実機ケース** | SilverStone RM42-502 | 5.25"ベイ×2(CD-ROM+FDD)。FLP02は不採用 |
| **MTG Shandalar** | 86Box（Pentium 100-133MHz設定）で対応 | 実機PII 400MHzでは速すぎる。cpukillerも非実用的 |
| **MTG実機の可能性** | PII 233MHz(Klamath)を保留 | 86Boxでテスト後に購入判断。速すぎる可能性あり(IPC差) |
| **MM6の3Dカード** | 不要（G200で十分） | MM6はソフトウェアレンダリング専用 |
| **86Box（仮想環境）** | MTGのプライマリプラットフォーム | CPUクロック設定が自在。速度問題の根本解決 |
| **DOSBox-X** | 第2候補 | cycles設定が最も手軽で直感的 |
...
- [x] パーツ選定・購入ガイド (cmd_262, cmd_295-296, 2026-03-25)
- [x] ケース互換性調査 — RM42-502に決定 (cmd_301, cmd_303, 2026-03-26)
- [x] PS/2デバイス調査 (cmd_300, cmd_302, 2026-03-26)
- [x] 光学ドライブ・FDD調査 (cmd_305, 2026-03-27)
- [x] CPUクロック・スイートスポット調査 (cmd_306-308, 2026-03-27)
- [x] GPU/3Dアクセラレータ調査 — G200で十分と判明 (cmd_309, 2026-03-27)
- [x] **実機構成確定**: PII 400MHz + G200 + 440BX (2026-03-27)
- [ ] 残りパーツ購入（マザーボード、キーボード、ドライブ類、ケーブル等）
- [ ] 86Boxセットアップ + MTG Shandalar速度テスト
- [ ] 実機組み立て（Phase 2）
- [ ] MM6動作確認
- [ ] MTG Shandalar動作確認（86Box）

## 注意事項
- Windows 98のインストールメディア・ライセンスは殿が所有済み（前回使用実績あり）
- Might and Magic VIは購入済み
- ゲームソフトの入手手段: abandonware（My Abandonware, Internet Archive）で入手可能（権利関係は複雑）
- **次のアクション**: 86Boxセットアップ → MTGの速度テスト → PII 233MHz購入判断
