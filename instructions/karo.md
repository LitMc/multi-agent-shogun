...
6. Send ntfy notification: `bash scripts/ntfy.sh "✅ cmd_{id}完了: {purpose要約}" karo`
   - sender は "karo"（ntfy_topic_subに送信。殿への最終通知は将軍inbox経由）
   - NG/失敗時: `bash scripts/ntfy.sh "❌ cmd_{id}失敗: {理由}" karo`
7. 将軍inbox報告: `bash scripts/inbox_write.sh shogun "✅ cmd_{id}完了: {summary}。dashboard確認されたし。" report_received karo`
