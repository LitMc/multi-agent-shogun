...
if [ "$STOP_HOOK_ACTIVE" = "True" ]; then
    ...
    # All agents: if unread messages exist when going idle, self-nudge.
    # This covers the case where inbox_watcher's nudge was lost during
    # Working (send-keys to busy Claude Code is unreliable).
    # Previously shogun-only; extended to all agents to fix RC-1.
    SELF_INBOX="$SCRIPT_DIR/queue/inbox/${AGENT_ID}.yaml"
    if [ -f "$SELF_INBOX" ]; then
        SELF_UNREAD=$(grep -c 'read: false' "$SELF_INBOX" 2>/dev/null || true)
        if [ "${SELF_UNREAD:-0}" -gt 0 ] && [ -n "${TMUX_PANE:-}" ]; then
            (sleep 1 && tmux send-keys -t "$TMUX_PANE" "inbox${SELF_UNREAD}" 2>/dev/null && sleep 0.3 && tmux send-keys -t "$TMUX_PANE" Enter 2>/dev/null) &
        fi
    fi
